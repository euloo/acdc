...
# main routine
def ParamAP(RAW_XY, UNIT_X="ms", UNIT_Y="mV'", AVG_FRAME=1000):
    ...
    while True:  # repeat data analysis for current file
        ...
        while True:  # time series analysis
            try:
                ...
                if startpdf:
                    pdf_file = mpbp.PdfPages(name+".pdf", keep_empty=False)  # multi-pdf file
                ...
                if APMODE:
                    ...
                    np.savetxt(name + "_" + timestamp + "_avg.dat", np.column_stack((avg_x, avg_y, avgf_y)), fmt='%e', delimiter='\t', header=uheader)
                    mpp.tight_layout()
                    mpp.savefig(pdf_file, format='pdf', dpi=600)
                    #sum_file = os.path.join(WORKDIR, "ParamAP.log")
                    #newfile = not bool(os.path.exists(sum_file))
                    #with open("ParamAP.log", 'a') as targetfile:  # append file
